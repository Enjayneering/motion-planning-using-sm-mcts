...
def plot_reachable_points(ax, array, config, game_length=None, color_0='red', color_1='blue'):
    init_state = get_init_state(config["env_raceconfig"])
    max_vel_0 = np.max(config["velocity_0"])
    max_vel_1 = np.max(config["velocity_1"])

    for row in range(len(array)):
        for col in range(len(array[0])):
            # agent 0 color
            if is_reachable_point([init_state['x0'], init_state['y0']], max_vel_0, [col, row], game_length):
                circle = Circle((col, row), radius=0.4, facecolor='none', edgecolor=color_0, zorder=2)
                ax.add_patch(circle)
                #ax.plot(col, row, 'o', color=color_0, markersize=4)
            # agent 1 color
            if is_reachable_point([init_state['x1'], init_state['y1']], max_vel_1, [col, row], game_length):
                circle = Circle((col, row), radius=0.3, facecolor=color_1, edgecolor=color_1, zorder=2)
                ax.add_patch(circle)
                #ax.plot(col, row, 'o', color=color_1, markersize=2)
