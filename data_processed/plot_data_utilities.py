...
def plot_visits_map(ax, config, result_file_path, env_timestep=None):
    ...
    plot_centerline(ax, centerlines=Env.centerlines, agent=0, color=colormap['red'])
    plot_centerline(ax, centerlines=Env.centerlines, agent=1, color=colormap['blue'])

    # plot initial state
    #annotate_state(ax, Env.init_state[0], Env.init_state[1], Env.init_state[2], 0, agent=0, annotate=False, facecolor=colormap['red'], edgecolor=colormap['darkred'], fontsize=4, zorder=100, alpha=1)
    #annotate_state(ax, Env.init_state[3], Env.init_state[4], Env.init_state[5], 0, agent=1, annotate=False, facecolor=colormap['blue'], edgecolor=colormap['darkblue'], fontsize=4, zorder=100, alpha=1)
    
    # PLOT VISITED POSITIONS
    result_data = pd.read_csv(result_file_path)
    
    # Extract coordinates from the trajectory columns for each agent
    def extract_coordinates(data, agent_col):
        positions = []
        for index, row in data.iterrows():
            trajectory = ast.literal_eval(row[agent_col])
            for point in trajectory:
                positions.append((point[0], point[1]))
        return positions

    # Define colors for each agent
    colors = [colormap['red'], colormap['blue']]

    # Process and plot positions for each agent
    for agent in range(2):  # Assuming two agents: 0 and 1
        agent_col = f'trajectory_{agent}'
        positions = extract_coordinates(result_data, agent_col)

        # Count the frequency of each position being visited
    ...
    def extract_collision_positions(data):
        collision_positions = []
        for index, row in data.iterrows():
            # Check if collision payoff is 0.0 for any agent
            if row['collisions'] == 1 :
                # Extract positions from trajectories where collisions occurred
                point_x = []
                point_y = []
                for col in ['trajectory_0', 'trajectory_1']:
                    trajectory = ast.literal_eval(row[col])
                    for point in trajectory:
                        point_x.append(point[0])
                        point_y.append(point[1])
                collision_positions.append((int(np.mean(point_x)), int(np.mean(point_y)), row['run_num']))
        # delete all duplicates and only return positions without run_num
        collision_positions = list(set(collision_positions))
        collision_positions = [(x, y) for x, y, _ in collision_positions]
        return collision_positions

    # Extract collision positions
    collision_positions = extract_collision_positions(result_data)

    print(collision_positions)

    if len(collision_positions) != 0:
        # Count the frequency of each collision position
        collision_counts = pd.Series(collision_positions).value_counts().reset_index()
        collision_counts.columns = ['position', 'count']
        collision_counts[['x', 'y']] = pd.DataFrame(collision_counts['position'].tolist(), index=collision_counts.index)

        #print(collision_counts)

        # Plot the collision positions with a red cross and label the count
        for i, row in collision_counts.iterrows():
...
def plot_config_map(ax, config, env_timestep=None, game_length=None):
    colormap = {'red': (192/255, 67/255, 11/255), 
                'darkred': (83/255, 29/255, 0/255),
                'blue': (78/255, 127/255, 141/255), 
                'darkblue': (38/255, 57/255, 63/255),
...
def get_reachable_states(init_state, vel_list, yaw_rate_list, grid, game_length, dt=1):
    ...
    visited_flipped = visited #np.flip(visited, axis=1)
...
def plot_reachable_points(ax, array, config, game_length=None, color_0='red', color_1='blue'):
    ...
    reachable_states_0 = get_reachable_states([init_state[0], init_state[1], init_state[2]], config["0"]["velocity"], config["0"]["ang_velocity"], array, game_length, dt=1)
    reachable_states_1 = get_reachable_states([init_state[3], init_state[4], init_state[5]], config["1"]["velocity"], config["1"]["ang_velocity"], array, game_length, dt=1)
...
def plot_single_run(Game, path_savefig, result_dict= None, timestep=None, main_agent=0):
    ...
    plot_centerline(ax, centerlines=Game.env.centerlines, agent=0, color=colormap['red'])
    plot_centerline(ax, centerlines=Game.env.centerlines, agent=1, color=colormap['blue'])
