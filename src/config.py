import numpy as np
from config_utilities import *

env_dict = {
    'racetrack-7x16': {
        'env_def': {
            0: """
                ################
                ................
                ................
                .0.1............
                ................
                ................
                ################""",
        },
        'theta_0_init': 0,
        'theta_1_init': 0,
        'finish_line': 12,
    },
    'racetrack-4x13': {
        'env_def': {
            0: """
                #############
                ..0..........
                1............
                #############""",
            3: """
                #############
                ....+........
                ....+........
                #############""",
        },
        'theta_0_init': 0,
        'theta_1_init': 0,
        'finish_line': 12,
    },
    'racetrack-7x13': {
        'env_def': {
            0: """
                #############
                ####0#.......
                ####.........
                ####.#.......
                .1...........
                #############""",
            5: """
                #############
                ####.#.......
                ####.+.......
                ####.#.......
                .....+.......
                #############""",
        },
        'theta_0_init': np.pi/2,
        'theta_1_init': 0,
        'finish_line': 12,
    },
    'overtaking_no_obs': {
        'env_def': {
            0: """
                ################
                .0...............
                ...............
                .1...............
                ################""",
        },
        'theta_0_init': 0,
        'theta_1_init': 0,
        'finish_line': 12,
    },
    'lane_merging': {
        'env_def': {
            0: """
                ########################
                #1.....................#
                #####.....##############
                ###.....################
                #..0...#################
                ########################""",
        },
        'theta_0_init': 0,
        'theta_1_init': 0,
        'finish_line': 12,
    },
    'closing_door': {
        'env_def': {
            0: """
                ###############
                .0.....########
                ...............
                .1.....########
                ###############""",
            5: """
                ###############
                .......########
                .......+.......
                .......########
                ###############""",
        },
        'theta_0_init': 0,
        'theta_1_init': 0,
        'finish_line': 12,
    },
}

default_dict = {
    'terminal_progress': float, # estimator of game length based on tracksize and agents maximum speed
    'rollout_length': int, # length of rollout trajectory
    'alpha_t': 1, # defines ratio of game timehorizon with respect to a heuristic estimator for timesteps needed to reach the end of the track
    'final_move_depth': 1,

    'num_sim': 1,
    'num_iter': 1000,
    'delta_t': 1,
    'c_param': np.sqrt(2),

    'collision_distance': 0.5,

    'risk_factor': 0.5,
    'discount_factor': 0.9,

    'penalty_distance': float,
    'reward_progress_0': float,
    'reward_progress_1': float,
    'penalty_timestep_0': float,
    'penalty_timestep_1': float,
    'reward_lead_0': float,
    'reward_lead_1': float,

    'velocity_0': list,
    'ang_velocity_0': list,
    'velocity_1': list,
    'ang_velocity_1': list,
'feature_flags': {
        'final_move': {'robust-joint': False, 'robust-seperate': False, 'robust-depth': True, 'best': False, 'ucb': False},
        'collision_handling': {'punishing': True, 'pruning': False},
        'selection_policy': {'ucb': True, 'greedy': False, 'regret-matching': False},
        'rollout_policy': {'uniform_random': True, 'best': False},
        'payoff_weights': {'fixed': True, 'adaptive': False},
        'expansion_policy': {'every-child': True, 'random': False},
    }
    }
default_config = Config(default_dict)

# CONFIGURATIONS
test_dict = {
    'terminal_progress': 20,
    'alpha_t': 1,
    'num_sim': 1,
    'num_iter': 400,
    'delta_t': 1,
    'risk_factor': 0.5, #[0,1]
    'penalty_distance': -0.2,
    'reward_progress_0': 0,
    'reward_progress_1': 0,
    'penalty_agressor_0': 0,
    'penalty_agressor_1': 0,
    'penalty_timestep_0': -0.2,
    'penalty_timestep_1': -0.2,
    'reward_lead_0': 1,
    'reward_lead_1': 1,
    'velocity_0': np.linspace(0, 2, 3).tolist(),
    'ang_velocity_0': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    'velocity_1': np.linspace(0, 2, 3).tolist(),
    'ang_velocity_1': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    
}

exp_weigths_dict = {
    'terminal_progress': 6,
    'num_sim': 2,
    'num_iter': 200,
    'delta_t': 1,
    'risk_factor': 0.5, #[0,1]
    'penalty_distance': -1,
    'reward_progress_0': 0,
    'reward_progress_1': 0,
    'penalty_timestep_0': -0.1,
    'penalty_timestep_1': -0.1,
    'reward_lead_0': 1,
    'reward_lead_1': 1,
    'velocity_0': np.linspace(0, 2, 3).tolist(),
    'ang_velocity_0': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    'velocity_1': np.linspace(0, 1, 2).tolist(),
    'ang_velocity_1': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
}

exp_iter_dict_01 = {
    'finish_line': 12,
    'rollout_length': 8,
    'terminal_progress': 8,

    'num_sim': 5,
    'num_iter': 1000,
    'risk_factor': 0.5, #[0,1]
    'discount_factor': 0.9,
    'penalty_distance': -1,
    'reward_progress_0': 0.1,
    'reward_progress_1': 0.1,
    'penalty_timestep_0': -0.1,
    'penalty_timestep_1': -0.1,
    'reward_lead_0': 1,
    'reward_lead_1': 1,

    'velocity_0': np.linspace(0, 2, 3).tolist(),
    'ang_velocity_0': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    'velocity_1': np.linspace(0, 1, 2).tolist(),
    'ang_velocity_1': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
}

exp_iter_dict_02 = {
    'rollout_length': 10,
    'terminal_progress': 10,
    'alpha_t': 1,
    'num_sim': 10,
    'num_iter': 1000,
    'delta_t': 1,
    'risk_factor': 0.8, #[0,1]
    'discount_factor': 0.9,
    'penalty_distance': -1,
    'reward_progress_0': 0.5,
    'reward_progress_1': 0.5,
    'penalty_agressor_0': -0,
    'penalty_agressor_1': -0,
    'penalty_timestep_0': -0.1,
    'penalty_timestep_1': -0.1,
    'reward_lead_0': 1,
    'reward_lead_1': 1,
    'velocity_0': np.linspace(0, 2, 3).tolist(),
    'ang_velocity_0': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    'velocity_1': np.linspace(0, 1, 2).tolist(),
    'ang_velocity_1': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
}

exp_overtaking_pruningcoll = {
    'c_param': np.sqrt(2),

    'rollout_length': 5,
    'terminal_progress': 10,

    'alpha_t': 1,
    'num_sim': 5,
    'num_iter': 1000,
    'delta_t': 1,
    'risk_factor': 1, #[0,1]
    'discount_factor': 0.8,

    'penalty_distance': -10,
    'reward_progress_0': 0.1,
    'reward_progress_1': 0.1,
    'penalty_timestep_0': -0.1,
    'penalty_timestep_1': -0.1,
    'reward_lead_0': 1,
    'reward_lead_1': 1,
    'velocity_0': np.linspace(0, 2, 3).tolist(),
    'ang_velocity_0': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    'velocity_1': np.linspace(0, 1, 2).tolist(),
    'ang_velocity_1': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    'feature_flags': {
        'final_move': {'robust-joint': False, 'robust-seperate': True, 'best': False, 'ucb': False},
        'collision_handling': {'punishing': False, 'pruning': True},
        'selection_policy': {'ucb': True, 'greedy': False, 'regret-matching': False},
        'rollout_policy': {'uniform-random': True, 'best': False},
        'payoff_weights': {'fixed': True, 'adaptive': False},
        'expansion_policy': {'every-child': True, 'random': False},
    }
}

exp_overtaking_punishcoll = {
    'c_param': np.sqrt(2),
    'final_move_depth': 3,

    'rollout_length': 5,
    'terminal_progress': 10,

    'alpha_t': 1,
    'num_sim': 5,
    'num_iter': 3000,
    'delta_t': 1,
    'risk_factor': 1, #[0,1]
    'discount_factor': 0.8,

    'penalty_distance': -10,
    'reward_progress_0': 0.1,
    'reward_progress_1': 0.1,
    'penalty_timestep_0': -0.1,
    'penalty_timestep_1': -0.1,
    'reward_lead_0': 1,
    'reward_lead_1': 1,
    'velocity_0': np.linspace(0, 2, 3).tolist(),
    'ang_velocity_0': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    'velocity_1': np.linspace(0, 1, 2).tolist(),
    'ang_velocity_1': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    'feature_flags': {
        'final_move': {'robust-joint': False, 'robust-seperate': True, 'best': False, 'ucb': False},
        'collision_handling': {'punishing': True, 'pruning': False},
        'selection_policy': {'ucb': True, 'best': False},
        'rollout_policy': {'uniform_random': True, 'best': False},
        'payoff_weights': {'fixed': True, 'adaptive': False},
        'expansion_policy': {'every-child': True, 'random': False},
    }
}

exp_iter_dict_closing_door = {
    'theta_0_init': 0,
    'theta_1_init': 0,
    'finish_line': 12,

    'terminal_progress': 7,
    'alpha_t': 1,

    'num_sim': 5,
    'num_iter': 1000,
    'delta_t': 1,
    'risk_factor': 0.1, #[0,1]
    'discount_factor': 0.9,
    'penalty_distance': -1,
    'reward_progress_0': 0.1,
    'reward_progress_1': 0.1,
    'penalty_timestep_0': -0.1,
    'penalty_timestep_1': -0.1,
    'reward_lead_0': 1,
    'reward_lead_1': 1,

    'velocity_0': np.linspace(0, 2, 3).tolist(),
    'ang_velocity_0': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
    'velocity_1': np.linspace(0, 2, 3).tolist(),
    'ang_velocity_1': np.linspace(-np.pi/2, np.pi/2, 3).tolist(),
}

experimental_mode = False 



