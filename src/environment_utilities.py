...
class Environment:
    ...
    def __init__(self, config=None):
        ...
        self.init_state = self.get_init_state(config.env_raceconfig)
        self.goal_state = self.get_goal_state(config.env_raceconfig)
    ...
    def _get_closest_gridpoint(self, point=None, list_of_points=None):
    ...
    def get_init_state(self, env_raceconfig):
        init_state = {}
        for agent in range(0,2):
            ...
            closest_x_next, closest_y_next = self._get_closest_gridpoint(point=[agent_x_init[0], agent_y_init[0]], list_of_points= agent_next_list)
    ...
    def get_goal_state(self, env_raceconfig):
        goal_state = {}
        for agent in range(0,2):
            grid = env_raceconfig[f'{agent}']
            if any([char in grid for char in ['G']]):
                ...
                closest_x_prev, closest_y_prev = self._get_closest_gridpoint(point=[agent_x_goal[0], agent_y_goal[0]], list_of_points= agent_previous_list)
    ...
    def get_finish_line(self, env_def):
        ...
        if any([char in grid for char in ['F']]):
            ...
            occupancy_grid_define = grid.replace('.', '9').replace('#', '9').replace('F', '1')
            lines = [line.replace(' ', '') for line in occupancy_grid_define.split('\n') if line]
            transformed_grid = [list(map(int, line)) for line in lines]
            occupancy_grid = np.array(transformed_grid)     

            finish_line = int(np.where(occupancy_grid == 1)[1]) #column value
        ...
        for agent in range(0,2):
            ...
            if any([char in grid for char in ['x']]):
                ...
                while len(centerline_coordinates) > 0:
                    ...
                    point_x, point_y = self._get_closest_gridpoint(point=[point_x, point_y], list_of_points= centerline_coordinates)
    ...
    def get_occupancy_grid(self, grid):
        occupancy_grid_define = grid.replace('.', '0').replace('0', '0').replace('1', '0').replace('#', '1').replace('+', '2').replace('O', '0').replace('I', '0').replace('F', '0')
        lines = [line.replace(' ', '') for line in occupancy_grid_define.split('\n') if line]
        transformed_grid = [list(map(int, line)) for line in lines]
        occupancy_grid = np.array(transformed_grid)
