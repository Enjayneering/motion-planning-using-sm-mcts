...
def get_intermediate_payoffs(Game, prev_state_obj, next_state_obj, discount_factor=0):
    ...
    discount = 1/((1+discount_factor)**prev_state_obj.timestep)
    ...
    if Game.config.weight_progress > 0:
        progress_0 = get_agent_progress(Game, prev_state_obj.get_state(agent=0), next_state_obj.get_state(agent=0))
        progress_1 = get_agent_progress(Game, prev_state_obj.get_state(agent=1), next_state_obj.get_state(agent=1))
        
        weight_progress_0 = Game.config.weight_progress
        weight_progress_1 = Game.config.weight_progress

        # normalizing bounds
        max_agent_progress_0 = get_agent_progress(Game, prev_state_obj.get_state(agent=0), max_next_state_0)
        max_agent_progress_1 = get_agent_progress(Game, prev_state_obj.get_state(agent=1), max_next_state_1)
    ...
    if Game.config.weight_collision > 0:
        ...
        for i in range(1, num_coll_samples):
            intermediate_x = prev_state_0[0] + (next_state_0[0] - prev_state_0[0]) * i / 4
            intermediate_y = prev_state_0[1] + (next_state_0[1] - prev_state_0[1]) * i / 4
        ...
        for i in range(1, num_coll_samples):
            intermediate_x = prev_state_1[0] + (next_state_1[0] - prev_state_1[0]) * i / 4
            intermediate_y = prev_state_1[1] + (next_state_1[1] - prev_state_1[1]) * i / 4
...
def get_total_payoffs(Game, interm_payoff_list=None, final_payoff_list=None):
    ...
    min_total_interm = 0
    max_total_interm = np.sum([1/((1+Game.config.discount_factor)**k) for k in range(len(interm_payoff_list))]) # correct
