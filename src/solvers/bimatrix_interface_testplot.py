...
def run_bimatrix_interface(ix_agent, curr_state, env_conf, agent_conf, model_conf, algo_conf, timestep_sim=1):
    ...
    game_dict =  {
        ...
        'feature_flags': {
            'run_mode': {'test': True, 'exp': True, 'live-plot': False},
    ...
    BiMatrixGame = BiMatrixMCTS(Environment(Config(env_conf)), Config(game_dict), init_state=curr_state)
    # Run MCTS
    result_dict, policy_df, sampled_trajectories = BiMatrixGame.run_game(timestep_sim) #timestep sim =1
