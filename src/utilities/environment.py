...
class Environment:
    ...
    def __init__(self, env_conf=None):
        self.name = env_conf["name"]
        self.dynamic_grid = self.init_dynamic_grid(env_conf["env_def"])
        self.init_state = get_init_state(env_conf["env_raceconfig"])
        self.goal_state = get_goal_state(env_conf["env_raceconfig"])
        self.finish_line = self.get_finish_line(env_conf["env_def"])
        self.centerlines = self.get_centerlines(env_conf["env_raceconfig"], metric_dist=0.5)
        self.env_centerline = self.get_env_centerline(env_conf["env_def"], metric_dist=0.5)

        self.x_min = self.get_x_min(env_conf["env_def"][0])
        self.y_min = self.get_y_min(env_conf["env_def"][0])
        self.x_max = self.get_x_max(env_conf["env_def"][0])
        self.y_max = self.get_y_max(env_conf["env_def"][0])
...
def get_init_state(env_raceconfig):
    ...
    for agent in range(0,2):
        ...
        init_state.append(agent_x_init[0])
        init_state.append(agent_y_init[0])
        init_state.append(theta_init)
